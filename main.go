...
func main() {
	...
	if err != nil {
		log.Printf("Failed to connect to database: %v. Switching to In-Memory Storage.", err)
		...
		if err := migrateModel(db, &models.User{}, "User"); err != nil {
			log.Printf("Database migration failed at User model: %v", err)
		}
		...
		if err := migrateModel(db, &models.Client{}, "Client"); err != nil {
			...
			log.Printf("Database migration failed at Client model: %v", err)
		}

		// Migrate AuthCode model
		if err := migrateModel(db, &models.AuthCode{}, "AuthCode"); err != nil {
			log.Printf("Database migration failed at AuthCode model: %v", err)
		}

		// Migrate RefreshToken model
		if err := migrateModel(db, &models.RefreshToken{}, "RefreshToken"); err != nil {
			log.Printf("Database migration failed at RefreshToken model: %v", err)
		}
	...
	// Add root for system operational check
	e.GET("/", func(c echo.Context) error {
		return c.String(200, "System Operational")
	})
