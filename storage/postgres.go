...
func (s *PostgresStorage) StoreClient(client *models.Client) error {
	...
	// Generate client credentials if not present (although service likely does it)
	if client.ClientID == "" {
...
func (s *PostgresStorage) StoreAuthCode(code, clientID string, userID uint, redirectURI string, codeChallenge, codeChallengeMethod string) error {
...
func (s *PostgresStorage) GetAuthCode(code string) *models.AuthCode {
	...
	// Do not filter by used here, because we want to return it to validate it first.
	// We check usage in MarkAuthCodeUsed.
	// However, we should check expiration.
	if err := s.db.Where("code = ? AND expires_at > ?", code, time.Now()).First(&authCode).Error; err != nil {
